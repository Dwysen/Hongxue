...
class OpenClassViewController : UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let vc1 = OpenVideoViewController()
        
        self.addChildViewController(vc1)
        
        let vc2 = OpenClassHistoryViewController()
    ...
    func setupTitleView(){
        let bgView = UIView()
        bgView.frame = CGRect(x: 0, y: 0, width:Common.screenWidth, height: kTitlesViewH)
        view.addSubview(bgView)
        
        //标签
        let titlesView = UIView()
        titlesView.backgroundColor = UIColor.brown
        titlesView.frame = CGRect(x: 0, y: 0, width: Common.screenWidth, height: kTitlesViewH)
        bgView.addSubview(titlesView)
        
        //底部红色指示器
        let indicatorView = UIView()
        indicatorView.backgroundColor = Common.mainRed
        ...
        for index in 0..<count{
            ...
            btn.setTitleColor(Common.mainRed, for: .disabled)
   ...
   @objc func titlesClick(_ btn:UIButton) {
...
extension OpenClassViewController:UIScrollViewDelegate {
